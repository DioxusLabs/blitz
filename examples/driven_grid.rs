                ...
                let mut button = rdom.create_node(NodeType::Element(ElementNode {
                    ...
                    attributes: [
                        ...
                        (
                            ...
                            format!("rgb({}, {}, {})", count * 10, 0, (x + y) * 10,).into(),
                        ),
                        (("width", "style").into(), "100%".to_string().into()),
                        (("height", "style").into(), "100%".to_string().into()),
                        (("flex-direction", "style").into(), "row".to_string().into()),
...
impl Driver for Test {
    fn update(&mut self, mut rdom: NodeMut<'_>) {
        let rdom = rdom.real_dom_mut();
    ...
    fn handle_event(&mut self, node: NodeMut, event: &str, _: Arc<EventData>, _: bool) {
        if event == "click" {
    ...
    fn poll_async(&mut self) -> std::pin::Pin<Box<dyn Future<Output = ()> + '_>> {
