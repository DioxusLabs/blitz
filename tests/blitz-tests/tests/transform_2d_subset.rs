...
#[test]
fn a_genuinely_3d_matrix_is_still_dropped() {
    // The boundary the fix must not cross: `rotateX` cannot be represented in
    // 2d, and mis-projecting it is worse than ignoring it while blitz has no
    // 3d support. This is the case `has_3d` was reaching for.
    let harness = Harness::from_html(
        r#"<html><body style="margin:0">
    ...
    assert_eq!(
        transform_coeffs(&harness, "#box"),
        None,
        "a matrix outside the two-dimensional subset must not be flattened"
    );
}
