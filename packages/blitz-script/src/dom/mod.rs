...
pub(crate) fn node_id_of_value(value: &JsValue) -> Option<usize> {
...
pub(crate) fn this_node_id(this: &JsValue) -> JsResult<usize> {
...
pub(crate) fn node_wrapper(ctx: &DomCtx, node_id: usize, _context: &mut Context) -> JsObject {
    ...
    let proto = {
        ...
        match doc.get_node(node_id).map(|node| &node.data) {
            Some(NodeData::Document) => protos.document.clone(),
...
pub(crate) fn node_or_null(ctx: &DomCtx, node_id: Option<usize>, context: &mut Context) -> JsValue {
    match node_id {
