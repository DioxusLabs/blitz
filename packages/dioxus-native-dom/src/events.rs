use blitz_dom::{BaseDocument, EventDriver, Node};
...
use crate::NodeId;
use crate::dioxus_document::DioxusEventHandler;
...
impl RenderedElementBacking for NodeHandle {
    ...
    fn set_focus(&self, focus: bool) -> Pin<Box<dyn Future<Output = MountedResult<()>>>> {
        // Run the DOM focusing steps synchronously, mirroring `element.focus()`/`.blur()`
        // on the web: the resulting blur/focusout/focus/focusin events are dispatched to
        // vdom event handlers before this method returns.
        let is_focussed = self.doc().get_focussed_node_id() == Some(self.node_id);
        let handler = DioxusEventHandler::new(self.runtime.clone());
        let mut doc = Rc::clone(&self.doc);
        let mut driver = EventDriver::new(&mut doc, handler);
        if focus {
            driver.set_focus(self.node_id);
        } else if is_focussed {
