...
impl RenderedElementBacking for NodeHandle {
    ...
    fn set_focus(&self, focus: bool) -> Pin<Box<dyn Future<Output = MountedResult<()>>>> {
        // The focus/blur events generated by these calls are queued on the document
        // and dispatched the next time an event driver runs (e.g. when the containing
        // `DioxusDocument` is polled).
