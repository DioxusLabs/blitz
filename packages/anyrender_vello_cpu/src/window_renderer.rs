...
impl WindowRenderer for VelloCpuWindowRenderer {
    ...
    fn render<F: FnOnce(&mut Self::ScenePainter<'_>)>(&mut self, draw_fn: F) {
        ...
        let flush_time = start.elapsed().as_millis();
        let flush_ms = flush_time - command_time;
        ...
        let render_ms = render_time - flush_time;
        ...
        println!("Frame time: {overall_ms}ms (cmd: {command_ms}ms, flush: {flush_ms}ms, render: {render_ms}ms, swizel: {swizel_ms}ms, present: {present_ms}ms)");
    }
}
