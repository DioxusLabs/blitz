...
struct DecorationRunGeometry {
    /// The baseline of the decorating box on this line.
    baseline: f32,
    ascent: f32,
    descent: f32,
...
/// Baseline on `line` of the inline box generated by `node_id`, found by walking the Parley style
/// tree up from `run_style` (a descendant run of that box). Decorations are positioned relative
/// to the decorating box's own baseline, not the shifted baselines of its `vertical-align`ed
/// descendants. Falls back to the run's baseline if the node has no style on the chain.
fn decorating_box_baseline(
    line: &Line<'_, TextBrush>,
    styles: &[parley::Style<TextBrush>],
    run_style: u16,
    node_id: NodeId,
) -> f32 {
    let mut index = run_style;
    loop {
        let style = &styles[usize::from(index)];
        if style.brush.id == node_id {
            return line.style_baseline(index);
        }
        let parent = style.parent();
        if parent == index {
            return line.style_baseline(run_style);
        }
        index = parent;
    }
}
        ...
        for item in line.items() {
            if let PositionedLayoutItem::GlyphRun(glyph_run) = item {
                ...
                for entry in stack.iter() {
                    ...
                    let geometry = DecorationRunGeometry {
                        baseline: decorating_box_baseline(
                            &line,
                            styles,
                            glyph_run.style_index(),
                            entry.node_id,
                        ),
                        ..geometry.clone()
                    };
