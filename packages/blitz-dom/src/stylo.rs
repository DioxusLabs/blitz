...
use blitz_traits::events::{BlitzTransitionEvent, DomEvent, DomEventData};
use style::CaseSensitivityExt;
use style::animation::AnimationSetKey;
use style::animation::AnimationState;
use style::animation::Transition;
...
/// The lifecycle phase of a CSS transition that a [`DomEvent`] is reporting.
#[derive(Clone, Copy)]
enum TransitionPhase {
    /// The transition was created (`transitionrun`).
    Run,
    /// The transition entered its active phase, after any delay (`transitionstart`).
    Start,
    /// The transition completed (`transitionend`).
    End,
    /// The transition was canceled before completing (`transitioncancel`).
    Cancel,
}

/// Serialize a transition's pseudo-element as a CSS string (e.g. `"::before"`),
/// or the empty string if the transition runs on the element itself.
fn transition_pseudo_element_string(pseudo: Option<PseudoElement>) -> String {
    match pseudo {
        Some(pe) if pe.is_before() => "::before".to_string(),
        Some(pe) if pe.is_after() => "::after".to_string(),
        Some(PseudoElement::Marker) => "::marker".to_string(),
        _ => String::new(),
    }
}

/// Build a transition [`DomEvent`] for the given transition and lifecycle phase.
fn make_transition_event(
    ...
    let duration = transition.property_animation.duration;
    let elapsed_time = match phase {
        // `transitionend` always reports the full active duration.
        TransitionPhase::End => duration,
        // Other phases report the time spent in the active phase so far.
        _ => (now - transition.start_time).clamp(0.0, duration),
    } as f32;

    let data = BlitzTransitionEvent {
        property_name: transition
            ...
            .into_owned(),
        elapsed_time,
        pseudo_element: transition_pseudo_element_string(pseudo),
    };

    let data = match phase {
        TransitionPhase::Run => DomEventData::TransitionRun(data),
        TransitionPhase::Start => DomEventData::TransitionStart(data),
        TransitionPhase::End => DomEventData::TransitionEnd(data),
        TransitionPhase::Cancel => DomEventData::TransitionCancel(data),
    };

    DomEvent::new(node_id, data)
}
...
impl crate::document::BaseDocument {
    pub fn resolve_stylist(&mut self, now: f64) {
        ...
        // Transition events (transitionstart/end/run/cancel) generated by
        // ticking and reconciling the animation/transition state. Collected
        // here and queued on the document once the `sets` lock is released, so
        // that embedders can dispatch them after `resolve`.
        let mut transition_events: Vec<DomEvent> = Vec::new();
        ...
        for (key, set) in sets.iter_mut() {
            ...
            // Transitions canceled during the style traversal are about to be
            // removed by `clear_canceled_animations`; fire `transitioncancel`
            // for them first.
            for transition in set.transitions.iter() {
                if transition.state == AnimationState::Canceled {
                    transition_events.push(make_transition_event(
                        node_id,
                        key.pseudo_element,
                        transition,
                        TransitionPhase::Cancel,
                        now,
                    ));
                }
            }
            ...
            for transition in set.transitions.iter_mut() {
                // Transitions created during this traversal fire `transitionrun`.
                // If their delay has already elapsed, they also immediately
                // enter the active phase and fire `transitionstart`.
        ...
        // Queue the generated transition events for dispatch. We push directly
        // to the field (rather than via `queue_event`) to avoid a whole-`self`
        // mutable borrow that would conflict with the stylesheet guard borrow
        // held for the duration of this method.
        self.pending_events.extend(transition_events);
