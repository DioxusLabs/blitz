...
use super::{
    replaced::{ReplacedContext, replaced_measure_function},
    resolve_calc_value,
};
...
impl BaseDocument {
    ...
    fn compute_inline_layout_inner(
        ...
        for ibox in inline_layout.layout.inline_boxes_mut() {
            ...
            let iframe_replaced_output =
                self.nodes[node_id]
                    .data
                    .downcast_element()
                    .and_then(|element| {
                        let tag_name = &element.name.local;
                        if *tag_name != local_name!("iframe") {
                            return None;
                        }
            ...
            if style.position == Position::Absolute || is_floated {
                ...
                let output = iframe_replaced_output.unwrap_or_else(|| {
