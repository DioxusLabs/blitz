...
fn map_dom_event_to_ui_event(
    ...
    match event.data.clone() {
        ...
        // Transition events are generated by each document's own style
        // resolution, so they are not forwarded into sub-documents.
        DomEventData::TransitionRun(_) => None,
        DomEventData::TransitionStart(_) => None,
        DomEventData::TransitionEnd(_) => None,
        DomEventData::TransitionCancel(_) => None,
    }
}
...
pub(crate) fn handle_dom_event<F: FnMut(DomEvent)>(
    ...
    match &event.data {
        ...
        DomEventData::TransitionRun(_)
        | DomEventData::TransitionStart(_)
        | DomEventData::TransitionEnd(_)
        | DomEventData::TransitionCancel(_) => {
            // Do nothing (no default action)
        }
    }
